...
from aws_doc_sdk_examples_tools.agent.process_ailly_files import (
    process_ailly_files,
    VALUE_PREFIXES,
)
...
def create_sample_ailly_files(ailly_dir):
    # Sample file 1
    file1_path = ailly_dir / "example1.md.ailly.md"
...
"""
    ...
    # Sample file 2
    file2_path = ailly_dir / "example2.md.ailly.md"
...
def test_process_ailly_files(test_environment):
    ...
    process_ailly_files(str(ailly_dir), str(output_path))
    ...
    example1 = next((item for item in results if item.get("id") == "example1"), None)
    example2 = next((item for item in results if item.get("id") == "example2"), None)

    assert example1 is not None
    assert example2 is not None
    assert (
        example1["title"]
        == f"{VALUE_PREFIXES.get('title')}Grant Read-Only Access to an S3 Bucket"
    )
    assert (
        example2["title"]
        == f"{VALUE_PREFIXES.get('title')}Manage EC2 Instances in a Specific Region"
    )
